...
export default function CoinInfo({ open, onClose, onQuestsClick }) {
  if (!open) return null
    ...
    <Dialog open={open} onClose={onClose} className="fixed inset-0 z-[100] pointer-events-none">
      ...
      >
        <div className="flex flex-col items-center justify-between h-full">
          <h1 className='text-[18px] text-white font-bold text-center'>Количество TankCoin начисляемых за 1 клик по монете</h1>
          <p className='text-[13px] text-white font-bold text-center'>Выполнение заданий дает буст на некоторое время, буст увеличивает количество монет за 1 клик</p>
          <button
