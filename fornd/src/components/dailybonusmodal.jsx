...
export default function DailyBonusModal({ open, onClose, onQuestsClick }) {
  if (!open) return null
    ...
    <Dialog open={open} onClose={onClose} className="fixed inset-0 z-[100] pointer-events-none">
      ...
      >
        <div className="flex flex-col items-center justify-between h-full">
          <h1 className='text-[18px] text-white font-bold text-center'>Количество TankCoin начисляемых ежедневно на твой баланс</h1>
          <p className='text-[13px] text-white font-bold text-center'>Количество монет которые поступают на твой баланс ежедневно. С повышением уровня - повышается количество фарма</p>
          <button
